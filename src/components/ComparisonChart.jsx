  ...
  return (
    <div className="mb-8">
      ...
      <div className="flex items-center gap-3 mb-2">
        <span className="font-mono text-xs text-gray-500 w-20">Default</span>
        <div className="flex-1 bg-gray-100 rounded-full h-7 overflow-hidden">
          <motion.div className="h-full bg-red-400 rounded-full flex items-center justify-end pr-3" initial={{ width: 0 }} animate={{ width: `${defaultWidth}%` }} transition={{ duration: 1 }}>
            <span className="text-xs font-bold text-white">{defaultVal.toFixed(1)}{unit}</span>
          </motion.div>
        </div>
      </div>

      <div className="flex items-center gap-3 mb-2">
        <span className="font-mono text-xs text-orange-500 w-20">OpenViking</span>
        <div className="flex-1 bg-gray-100 rounded-full h-7 overflow-hidden">
          <motion.div className="h-full bg-orange-400 rounded-full flex items-center justify-end pr-3" initial={{ width: 0 }} animate={{ width: `${vikingWidth}%` }} transition={{ duration: 1, delay: 0.15 }}>
            <span className="text-xs font-bold text-white">{vikingVal.toFixed(1)}{unit}</span>
          </motion.div>
        </div>
      </div>

      <div className="flex items-center gap-3">
        <span className="font-mono text-xs text-clude w-20">Clude</span>
        <div className="flex-1 bg-gray-100 rounded-full h-7 overflow-hidden">
          <motion.div className="h-full bg-clude rounded-full flex items-center justify-end pr-3" initial={{ width: 0 }} animate={{ width: `${cludeWidth}%` }} transition={{ duration: 1, delay: 0.3 }}>
            <span className="text-xs font-bold text-white">{cludeVal.toFixed(1)}{unit}</span>
          </motion.div>
        </div>
      </div>
    </div>
...
export default function ComparisonChart({ results }) {
  ...
  return (
    <motion.div className="bg-white rounded-2xl p-8 shadow-sm" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      ...
      <div className="mt-8 p-6 bg-gray-50 rounded-xl">
        <h3 className="font-bold text-lg mb-4">Clude vs Competition</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div>
            ...
            <p className="text-xs text-gray-500 font-mono">Less hallucination vs Default</p>
          </div>
          <div>
            <p className="text-3xl font-bold text-clude">
              {vk.hallucinations > 0 ? `${(vk.hallucinations / Math.max(c.hallucinations, 0.1)).toFixed(0)}x` : '—'}
            </p>
            <p className="text-xs text-gray-500 font-mono">Less hallucination vs OpenViking</p>
          </div>
          <div>
            <p className="text-3xl font-bold text-clude">
