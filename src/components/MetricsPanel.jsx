...
function MetricRow({ label, defaultVal, cludeVal, unit = '', better = 'lower', highlight = false }) {
  const improvement = better === 'lower'
    ? defaultVal > 0 ? `${(defaultVal / Math.max(cludeVal, 0.01)).toFixed(0)}x` : '—'
    : cludeVal > 0 ? `+${(((cludeVal - defaultVal) / Math.max(defaultVal, 0.01)) * 100).toFixed(0)}%` : '—'

  return (
    <div className={`grid grid-cols-4 gap-4 py-4 border-b border-gray-100 ${highlight ? 'bg-blue-50/50 -mx-4 px-4 rounded-lg' : ''}`}>
      ...
      <p className="text-lg font-bold text-green-600 text-center">{improvement}</p>
    </div>
  )
}

export default function MetricsPanel({ defaultMetrics, cludeMetrics, running }) {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
    >
      <div className="grid grid-cols-4 gap-4 mb-4 pb-4 border-b-2 border-gray-200">
        ...
        <p className="font-mono text-xs text-green-600 tracking-wider text-center">IMPROVEMENT</p>
      </div>

      <MetricRow
        label="Hallucination Rate"
        defaultVal={d.hallucinations}
        cludeVal={c.hallucinations}
        unit="%"
        better="lower"
        highlight
      />
      <MetricRow
        ...
        defaultVal={d.factRetention}
        cludeVal={c.factRetention}
        unit="%"
        better="higher"
      />
      <MetricRow
        label="Prediction Accuracy"
        defaultVal={predAccDefault}
        cludeVal={predAccClude}
        unit="%"
        better="higher"
      />
      <MetricRow
